...
{
	for (int i = 0; strlen(keywords[i].keyword); i++)
		if (strlen(keywords[i].def))
			keywords[i].handler(keywords[i].def, keywords[i].var);
	
	read_ip("10.13.13.40", &(server_config.start));
	read_ip("10.13.13.60", &server_config.end);
	read_str("eth1", &(server_config.interface));
	return 1;
}
