...
{	
	...
	while(1) { /* loop until universe collapses */
		...
		case DHCPDISCOVER:
			...
			sprintf(outname, "./discover_%ld", time(0));
			testcase = fopen(outname, "wb");
			fwrite(&packet, sizeof(struct dhcpMessage), 1, testcase);
			fclose(testcase);
			DEBUG(LOG_INFO,"wrote testcase: %s", outname);
			exit_server(0);
 		case DHCPREQUEST:
			DEBUG(LOG_INFO, "received REQUEST");
			sprintf(outname, "./request_%ld", time(0));
			testcase = fopen(outname, "wb");
			fwrite(&packet, sizeof(struct dhcpMessage), 1, testcase);
			fclose(testcase);
			DEBUG(LOG_INFO,"wrote testcase: %s", outname);
		...
		case DHCPDECLINE:
			...
			sprintf(outname, "./decline_%ld", time(0));
			testcase = fopen(outname, "wb");
			fwrite(&packet, sizeof(struct dhcpMessage), 1, testcase);
			fclose(testcase);
			DEBUG(LOG_INFO,"wrote testcase: %s", outname);
		...
		case DHCPRELEASE:
			...
			sprintf(outname, "./release_%ld", time(0));
			testcase = fopen(outname, "wb");
			fwrite(&packet, sizeof(struct dhcpMessage), 1, testcase);
			fclose(testcase);
			DEBUG(LOG_INFO,"wrote testcase: %s", outname);
